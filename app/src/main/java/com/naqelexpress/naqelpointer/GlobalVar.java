...
//    }
    ...
    public void LoadMyRouteShipments(String orderBy, boolean CheckComplaintandDeliveryRequest, Context context, View view) {
        ...
        if (GlobalVar.GV().CourierDailyRouteID > 0) {
            Cursor result = dbConnections.Fill("select * from MyRouteShipments Where CourierDailyRouteID = " + GlobalVar.GV().CourierDailyRouteID + " order by " + orderBy, context);
            if (result.getCount() > 0) {
