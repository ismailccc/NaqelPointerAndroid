        ...
        extends MainActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        if(GlobalVar.GV().EmployID == 15380 || GlobalVar.GV().EmployID == -1 )//|| GlobalVar.GV().EmployID == 18110)
        ...
        btnExtractDB.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                try
                {
                    ...
                    if (!GlobalVar.GV().checkPermission(GlobalVar.GV().activity, GlobalVar.PermissionType.Storage) )
                    {
                        GlobalVar.GV().ShowSnackbar(GlobalVar.GV().rootView,GlobalVar.GV().activity.getString(R.string.NeedStoragePermision), GlobalVar.AlertType.Error);
                        GlobalVar.GV().askPermission(GlobalVar.GV().activity, GlobalVar.PermissionType.Storage);
                    }
                    else
                    {
                        if (sd.canWrite())
                        {
                            ...
                            if (currentDB.exists())
                            {
                                ...
                                src.close();
                                dst.close();
                                GlobalVar.GV().ShowSnackbar(mainRootView,"Extracted Successfully", GlobalVar.AlertType.Info);
                            }
                        }
                        else
                            GlobalVar.GV().ShowSnackbar(mainRootView,GlobalVar.GV().activity.getString(R.string.NeedStoragePermision), GlobalVar.AlertType.Info);
                    }
                }
                catch (Exception e){}
            }
