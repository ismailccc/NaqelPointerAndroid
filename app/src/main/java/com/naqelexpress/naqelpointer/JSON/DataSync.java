...
import com.naqelexpress.naqelpointer.JSON.Request.CheckPointWaybillDetailRequest;
...
import com.naqelexpress.naqelpointer.JSON.Request.GetShipmentForPickingRequest;
import com.naqelexpress.naqelpointer.JSON.Request.GetStationRequest;
...
import com.naqelexpress.naqelpointer.JSON.Request.SendCheckPointRequest;
import com.naqelexpress.naqelpointer.JSON.Request.UserMELoginRequest;
...
import com.naqelexpress.naqelpointer.JSON.Results.SendingDataResult;
import com.naqelexpress.naqelpointer.JSON.Results.StationResult;
...
public class DataSync
{
    ...
    public void SendUserMeLoginsData()
    {
        if (!GlobalVar.GV().HasInternetAccess)
            return;
        Cursor result = GlobalVar.GV().dbConnections.Fill("select * from UserMeLogin where IsSync = 0");
        if (result.getCount() > 0)
        {
            GlobalVar.GV().ShowSnackbar(GlobalVar.GV().rootViewMainPage,GlobalVar.GV().context.getString(R.string.SendingUserLoginData), GlobalVar.AlertType.Info);
            result.moveToFirst();
            do
            {
    ...
    public void SendNotDliveryData()
    {
        ...
        if(result.getCount() > 0 )
        {
            ...
            do
            {
                ...
                ProjectAsyncTask task = new ProjectAsyncTask("NotDelivered/Post", "Post",jsonData);
...
//                new SendNotDeliveredDataToServer().execute(jsonData);
    ...
    public void SendMultiDeliveryData()
    {
        ...
        if(result.getCount() > 0)
        {
            ...
            do
            {
                ...
                ProjectAsyncTask task = new ProjectAsyncTask("MultiDelivery/Post", "Post",jsonData);
                task.setUpdateListener(new OnUpdateListener()
                {
                    public void onPostExecuteUpdate(String obj)
                    {
    ...
    public void SendCheckPointData()
    {
        ...
        if(result.getCount() > 0)
        {
            ...
            do
            {
                ...
                ProjectAsyncTask task = new ProjectAsyncTask("CheckPoint/Post", "Post",jsonData);
                task.setUpdateListener(new OnUpdateListener()
                {
                    public void onPostExecuteUpdate(String obj)
                    {
    ...
    //-------------Check New Bookings -------------------
    public void CheckOnlineBooking()
    {
        if (!GlobalVar.GV().HasInternetAccess)
            return;
...
//        }
    ...
    private class GetUserMEDataFromServer extends AsyncTask<String,Void,String>
    {
        ...
        protected void onPreExecute()
        {
            progressDialog = ProgressDialog.show(GlobalVar.GV().context, "Please wait.", "Bringing User Details.", true);
        }
        ...
        protected void onPostExecute(String finalJson)
        {
            ...
            if (!getUserMEDataResult.HasError)
            {
                if (GlobalVar.GV().dbConnections != null)
                {
                    ...
                    GlobalVar.GV().dbConnections.deleteUserME(instance);
                    GlobalVar.GV().dbConnections.InsertUserME(instance);
                }
            }
            else
                GlobalVar.GV().ShowSnackbar(GlobalVar.GV().rootViewMainPage,"Data Not Sync Because :" + getUserMEDataResult.ErrorMessage, GlobalVar.AlertType.Error);
    ...
    private class GetDeliveryStatusFromServer extends AsyncTask<String,Void,String>
    {
        ...
        {
            //progressDialog.dismiss();
            new DeliveryStatus(finalJson,GlobalVar.GV().rootViewMainPage);
            super.onPostExecute(String.valueOf(finalJson));
        }
    }
...
//    }
    ...
    private class GetCheckPointTypeFromServer extends AsyncTask<String,Void,String>
    {
        ...
        {
            progressDialog.dismiss();
            new CheckPointTypeResult(finalJson,GlobalVar.GV().rootViewMainPage);
            super.onPostExecute(String.valueOf(finalJson));
        }
    }
